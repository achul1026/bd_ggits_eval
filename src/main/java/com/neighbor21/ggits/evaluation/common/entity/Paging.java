...
public class Paging {
    ...
    private int pageNoSize; // 페이지 개수(페이징 네비 기준)
    ...
    private void makePaging() {
        ...
        if (this.pageSize == 0) this.setPageSize(4); // 기본 값 설정
        if (this.pageNoSize == 0) this.setPageNoSize(5); // 기본 값 설정
        if (this.startPageNo == 0) { // 기본 값 설정
        	this.setStartPageNo(((pageNo - 1) / this.pageNoSize) * this.pageNoSize + 1);
        }
        ...
        int endPage = this.startPageNo + this.pageNoSize - 1; // 끝 페이지 (페이징 네비 기준)
