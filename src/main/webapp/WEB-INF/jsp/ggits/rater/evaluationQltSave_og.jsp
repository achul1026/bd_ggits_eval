...
<div class="content">
	...
	<div class="table-exel-btn-wrap">
		현재 평가 점수 <span id="currentScr" style="color: #266AE1;">0</span> / ${totalScr}점 &nbsp;&nbsp;&nbsp; 남은 문항 수 <span id="remainScrCount" style="color: #266AE1;"></span>
	</div>
	
	<div class="table-wrap test-table pc-table-qlt">
		<c:choose>
			<c:when test="${evalQltInfo.evalShtSctrList[0].evalShtItemList[0].itmNm ne null and evalQltInfo.evalShtSctrList[0].evalShtItemList[0].itmNm eq ''}">
	...
	<c:forEach var="evalShtSctrList" items="${evalQltInfo.evalShtSctrList}">
		<div class="table-body pc-table-qlt-body tablet-table-body ${isItmNmEmpty eq 'true' ? 'rater-three-qlt-body-width' : ''}">
			...
			<div class="table-list sub-body">
				<c:forEach var="evalShtItemList" items="${evalShtSctrList.evalShtItemList}">
					...
					<c:if test="${isItmNmEmpty eq 'false'}">
						...
						<div id="maxScr" data-max-scr="${evalShtItemList.evalShtQltScrInfo.fldScr}">
							<!-- 배점 -->
							${evalShtItemList.evalShtQltScrInfo.fldScr}점
						</div>
						<!-- 배점표 -->
							  ...
							  <div class="sign-wrap test-list-wrap" >
								  <div class="scoreDiv sign-content">
										...
										<div class="sign-body test-list-text check-list">
											...
											<ul class="test-table">
												<c:set var="fldScr" value="${evalShtItemList.evalShtQltScrInfo.fldScr}"/>
										...
										<div class="sign-btn">
											<button type="button" class="mini-btn mini-sub-btn on-btn wd100" onClick="fnScrTextChange('${scrCount }')">확인</button>
										</div>
								  </div>
								</div>
						</div>
		...
		if (saveType == 'save' || saveType == 'update') {
			...
			if(isAllScrChecked) {
				if($('#signed').val() == 'true') {
					...
					alert('서명이 완료되어야 평가를 제출할 수 있습니다.');
				}
			} else {
