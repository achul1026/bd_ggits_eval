...
<div class="login-content">
	...
	<div class="type-select-btn">
		...
		정성적 평가<c:if test="${shtExistCheck eq 'QNT_EMPTY' or shtExistCheck eq 'BOTH_EXIST'}"> (완료)</c:if>
		</button>
		<br>
		<button type="button" class="main-btn" onclick="fnEvalType('qnt')">
		정량적 평가<c:if test="${shtExistCheck eq 'QLT_EMPTY' or shtExistCheck eq 'BOTH_EXIST'}"> (완료)</c:if>
		</button>						
		<br>
	...
	function fnEvalType(evalType){
		if(evalType == 'qlt'){
			if(shtExistCheck == 'QNT_EMPTY'){
				alert("완료된 정성적 평가지 정보가 있습니다. 정량적 평가를 진행해 주세요.");
				return false;
			}
			...
			if(shtExistCheck == 'QLT_EMPTY'){
				alert("완료된 정량적 평가지 정보가 있습니다. 정성적 평가를 진행해 주세요.");
				return false;
			}
